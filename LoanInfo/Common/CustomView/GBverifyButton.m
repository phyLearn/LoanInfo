...
-(void)createUI{
    ...
    _get_verifyButton=[self createButtonWithFrame:self.bounds ImageName:nil Target:self.target Action:self.action Title:@"获取验证码"];
...
-(void)getVerifyClick{
    ...
    if([[NSUserDefaults standardUserDefaults]objectForKey:@"GBVerifyEnterAgain"]==NO){
        ...
        if(interval>60){
            ...
            [LoanInfoToast showToastWithMessage:@"您的操作过于频繁，请稍后再试" duration:2.0];
        }
    }
