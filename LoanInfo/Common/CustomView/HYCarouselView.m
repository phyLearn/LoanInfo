...
//  AppFramework
//
//  Created by Penghuaiyu on 16/10/18.
//  Copyright © 2016年 Penghuaiyu. All rights reserved.
//

#import "HYCarouselView.h"
...
@interface HYCarouselView()<UIScrollViewDelegate>
//轮播的图片数组
...
+ (instancetype)carouselViewWithImageArray:(NSArray *)imageArray describeArray:(NSArray *)describeArray {
    HYCarouselView *carouselView = [[self alloc] init];
...
- (void)setImageArray:(NSArray *)imageArray{
    ...
    for (int i = 0; i < imageArray.count; i++) {
        if ([imageArray[i] isKindOfClass:[UIImage class]]) {
            ...
            self.placeImageName?[_images addObject:[UIImage imageNamed:self.placeImageName]] : [_images addObject:[[UIImage alloc] init]];
